...
####################################################################
## This version does not detect or handle any hazards
...
# Since the next instruction is determined by f stage, not F-register, no bubble needs.
bool F_bubble = 0;

# If any forwarding stages need stall, so does the fetch stage.
# When considering data hazard, we have to handle RNONE case.
bool F_stall = 
    # RET
    IRET in { D_icode, E_icode, M_icode } ||
    # Register file data hazard && Load/Use hazard
    !(d_srcA == RNONE) && d_srcA in { E_dstM, e_dstE, M_dstM, M_dstE, W_dstM, W_dstE } ||
    !(d_srcB == RNONE) && d_srcB in { E_dstM, e_dstE, M_dstM, M_dstE, W_dstM, W_dstE };
    # Since no forwarding is implemented, whether the hazard is data hazard or load/use hazard is indifferent.
    # Both need to stall until concerned instructions pass write-back stage.
...
bool D_bubble =
    # RET && combination handling.
    # && takes precedence over ||.
    # We have no choice but to wait for %rsp to be updated at Write-back stage.
    (IRET in { D_icode, E_icode, M_icode } &&
    !(!(d_srcA == RNONE) && d_srcA in { E_dstM, e_dstE, M_dstM, M_dstE, W_dstM, W_dstE })) ||
    # Mispredicted Branch
    # == takes precedence over &&.
