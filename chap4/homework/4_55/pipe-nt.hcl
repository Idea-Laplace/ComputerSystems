...
wordsig D_ifun 'if_id_curr->ifun'     # Instruction function            # Added for this hcl.
wordsig D_rA 'if_id_curr->ra'	     # rA field from instruction
wordsig D_rB 'if_id_curr->rb'	     # rB field from instruction
wordsig D_valC 'if_id_curr->valc'    # ValC for branch taken.           # Added for this hcl.
...
word f_pc = [
	# Mispredicted branch. Fetch at incremented PC -> Fetch at jump address PC
	M_icode == IJXX && M_ifun != UNCOND && M_Cnd : M_valA;      # Before: M_icode == IJXX && !M_Cnd : M_valA
...
word f_predPC = [
	...
	f_icode == ICALL || f_icode == IJXX && f_ifun == UNCOND : f_valC;       # Before: f_icode in { ICALL, IJXX } : f_valC
	1 : f_valP;
];
...
word d_valA = [
	D_icode == ICALL || D_icode == IJXX && D_ifun == UNCOND : D_valP;       # Before:  D_icode in { ICALL, IJXX } : D_valP; # Use incremented PC
    D_icode == IJXX && D_ifun != UNCOND : D_valC;                           # Case for misprediction.
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && E_ifun != UNCOND && e_Cnd) ||                           # From !e_Cnd to e_Cnd, and added not UNCOND condtion
	# Stalling at fetch while ret passes through pipeline
...
bool E_bubble =
	# Mispredicted branch
	(E_icode == IJXX && E_ifun != UNCOND && e_Cnd) ||                          # Ditto.
	# Conditions for a load/use hazard
	E_icode in { IMRMOVQ, IPOPQ } &&
	 E_dstM in { d_srcA, d_srcB};
